...
GxGDEW042T2B::GxGDEW042T2B(GxIO& io, uint8_t rst, uint8_t busy)
  : GxEPD(GxGDEW042T2B_WIDTH, GxGDEW042T2B_HEIGHT),
...
void GxGDEW042T2B::drawPixel(int16_t x, int16_t y, uint16_t color)
{
  ...
  switch (getRotation())
  {
    case 1:
      swap(x, y);
      x = GxGDEW042T2B_WIDTH - x - 1;
      break;
    case 2:
      x = GxGDEW042T2B_WIDTH - x - 1;
      y = GxGDEW042T2B_HEIGHT - y - 1;
      break;
    case 3:
      swap(x, y);
      y = GxGDEW042T2B_HEIGHT - y - 1;
  ...
  uint16_t i = x / 8 + y * GxGDEW042T2B_WIDTH / 8;
...
void GxGDEW042T2B::fillScreen(uint16_t color)
{
  uint8_t data = (color == GxEPD_BLACK) ? 0xFF : 0x00;
  for (uint16_t x = 0; x < GxGDEW042T2B_BUFFER_SIZE; x++)
  {
    _buffer[x] = data;
  }
}
...
void GxGDEW042T2B::update(void)
{
  uint32_t i;
  uint8_t data;
  _wakeUp();
  IO.writeCommandTransaction(0x13);
  for (i = 0; i < GxGDEW042T2B_BUFFER_SIZE; i++)
  {
    data = _buffer[i];
...
void  GxGDEW042T2B::drawBitmap(int16_t x, int16_t y, const uint8_t *bitmap, int16_t w, int16_t h, uint16_t color)
{
  drawBitmap(bitmap, x, y, w, h, color);
}

void  GxGDEW042T2B::drawBitmap(const uint8_t *bitmap, uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t color, bool mirror)
{
  if (mirror)
  {
...
void GxGDEW042T2B::drawBitmap(const uint8_t *bitmap, uint32_t size)
{
  uint32_t i;
  uint8_t data;
  _wakeUp();
...
void GxGDEW042T2B::_waitWhileBusy(const char* comment)
{
...
void GxGDEW042T2B::_wakeUp(void)
{
  digitalWrite(_rst, 0);
  delay(100);
  digitalWrite(_rst, 1);
...
void GxGDEW042T2B::_writeLUT(void)
{
  uint16_t count;
...
void GxGDEW042T2B::greyTest() // what do the 2 channels provide ?
{
  _wakeUp();
  IO.writeCommandTransaction(0x10);
  for (uint16_t y = 0; y < GxGDEW042T2B_HEIGHT; y++)
  {
    for (uint16_t x8 = 0; x8 < GxGDEW042T2B_WIDTH / 8; x8++)
    {
      uint8_t data = (x8 < GxGDEW042T2B_WIDTH / 8 / 2) ? 0xFF : 0x00;
      IO.writeDataTransaction(data);
    }
  }
  IO.writeCommandTransaction(0x13);
  for (uint16_t y = 0; y < GxGDEW042T2B_HEIGHT; y++)
  {
    for (uint16_t x8 = 0; x8 < GxGDEW042T2B_WIDTH / 8; x8++)
    {
      uint8_t data = (y <  GxGDEW042T2B_HEIGHT / 2) ? 0xFF : 0x00;
      IO.writeDataTransaction(data);
    }
  }
  IO.writeCommandTransaction(0x12);      //display refresh
